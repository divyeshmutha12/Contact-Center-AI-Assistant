...
# Agent files root - must match data_extraction_agent.py
...
# ------------------------------------------------------
# File Download Endpoint (for agent-generated files)
# ------------------------------------------------------

@chat_bp.route("/download/<path:filename>", methods=["GET"])
def download_file(filename):
    """
    Download a file generated by the agent.

    This endpoint serves files from the agent_files directory.
    Files are created by the convert_ejson_to_excel tool.

    URL: /api/chat/download/outputs/report.xlsx

    Args:
        filename: Path to file relative to agent_files (e.g., outputs/report.xlsx)
    ...
    try:
        ...
        # Normalize the path and ensure it stays within AGENT_FILES_ROOT
        safe_path = os.path.normpath(filename)
        if safe_path.startswith('..') or safe_path.startswith('/'):
            logger.warning(f"Path traversal attempt blocked: {filename}")
            abort(403)

        file_path = os.path.join(AGENT_FILES_ROOT, safe_path)

        # Verify the resolved path is still within AGENT_FILES_ROOT
        if not os.path.abspath(file_path).startswith(os.path.abspath(AGENT_FILES_ROOT)):
            logger.warning(f"Path traversal attempt blocked: {filename}")
            abort(403)
